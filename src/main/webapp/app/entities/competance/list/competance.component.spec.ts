import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { CompetanceService } from '../service/competance.service';

import { CompetanceComponent } from './competance.component';

describe('Competance Management Component', () => {
  let comp: CompetanceComponent;
  let fixture: ComponentFixture<CompetanceComponent>;
  let service: CompetanceService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'competance', component: CompetanceComponent }]), HttpClientTestingModule],
      declarations: [CompetanceComponent],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: {
            data: of({
              defaultSort: 'id,asc',
            }),
            queryParamMap: of(
              jest.requireActual('@angular/router').convertToParamMap({
                page: '1',
                size: '1',
                sort: 'id,desc',
              })
            ),
            snapshot: { queryParams: {} },
          },
        },
      ],
    })
      .overrideTemplate(CompetanceComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(CompetanceComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(CompetanceService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.competances?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to competanceService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getCompetanceIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getCompetanceIdentifier).toHaveBeenCalledWith(entity);
      expect(id).toBe(entity.id);
    });
  });
});
