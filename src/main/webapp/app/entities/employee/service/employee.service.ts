...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'dateembauche'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      dateembauche: employee.dateembauche?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      dateembauche: restEmployee.dateembauche ? dayjs(restEmployee.dateembauche) : undefined,
    };
  }
