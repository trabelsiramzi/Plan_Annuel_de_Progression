import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { EntretienService } from '../service/entretien.service';

import { EntretienComponent } from './entretien.component';

describe('Entretien Management Component', () => {
  let comp: EntretienComponent;
  let fixture: ComponentFixture<EntretienComponent>;
  let service: EntretienService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'entretien', component: EntretienComponent }]), HttpClientTestingModule],
      declarations: [EntretienComponent],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: {
            data: of({
              defaultSort: 'id,asc',
            }),
            queryParamMap: of(
              jest.requireActual('@angular/router').convertToParamMap({
                page: '1',
                size: '1',
                sort: 'id,desc',
              })
            ),
            snapshot: { queryParams: {} },
          },
        },
      ],
    })
      .overrideTemplate(EntretienComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(EntretienComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(EntretienService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.entretiens?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to entretienService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getEntretienIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getEntretienIdentifier).toHaveBeenCalledWith(entity);
      expect(id).toBe(entity.id);
    });
  });
});
